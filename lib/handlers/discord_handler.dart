...
class DiscordHandler extends ReportHandler {
  ...
  Future<bool> handle(Report error, BuildContext? context) async {
    ...
    for (final value in messages) {
      ...
      final bool result = await _sendContent(value, isLastMessage ? error.screenshot : null);
      if (!result) {
        return result;
      }
    }
  ...
  Future<bool> _sendContent(String content, File? screenshot) async {
    try {
      ...
      if (screenshot != null) {
        ...
        final FormData formData = FormData.fromMap(<String, dynamic>{"content": content, "file": await MultipartFile.fromFile(screenshotPath)});
